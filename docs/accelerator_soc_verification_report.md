...
> 本报告记录已完成的工作、遇到的问题与解决方法、当前仍未解决的问题，以及
> 对应的官方文档来源。
...
| TUM eikon 实验服务器 | QuestaSim 2023.4（4-state，全 SoC + JTAG） | 流程全部打通，卡在 SoC 取指 X 传播（见 §5） |
...
方法对齐 `scripts/c_code/main_gemm.c` 的随机输入 + 预计算黄金参考范式：

- 新增生成器 `scripts/c_code/gen_accel_data.py`（numpy，固定种子 `0xACCE`），
...
### 5.1 QuestaSim 全 SoC 取指路径的 X 传播（根因在 SoC/厂商 IP 基础设施）

**现象**：内核 `resume` 后开始执行，但在
`PC 0x0100_023c` 取到指令 **`0xX0XXX7XX`**（部分位为 4-state 未知值 X），随即触发
非法指令异常并自旋。
...
- Ibex 取指（IF）阶段使用一个 **prefetch buffer（FIFO）**；且**非对齐 / 压缩指令**
  需要**两次字对齐访问拼接**成 32 位指令（前一次取低 16 位、后一次取高 16 位）
  —— 见 Ibex 官方文档（§6）。这正好解释 `0xX0XXX7XX` 这种「**半字为 X**」的部分
  未知值：prefetch FIFO 中**未复位的触发器**在 4-state 仿真下上电为 X，拼接后污染
  取到的指令。
- 这是 **4-state 仿真的 X 悲观性（X-pessimism）**：真实硅片 / FPGA 上这些触发器上电
  是确定的 0/1，功能正确；2-state 的 Verilator 默认按 0 处理，也正确；唯独 4-state
  的 QuestaSim 把它们当作 X。

**已尝试的「不改 RTL」规避（无效）**：

- 在 `vopt` 阶段加 QuestaSim 的确定性初始化选项 `+initreg+0 +initmem+0`
  （语法见 §6，从工具二进制 help 提取确认）。
- 结果：选项**被接受**、进入 vopt 命令行、elaboration 通过，但
  1. 取指 X **完全没有改变**（仍为 `0xX0XXX7XX`）；
  2. 反而触发大批 Ibex 的 `ASSERT_KNOWN` X 检查断言（`IbexDataRPayloadX`、
     `IbexBranchDecisionValid`、`Ibex…KnownEnable` 等，集中在 id_stage），并使
     `vsim` 崩溃（`Trouble with Simulation Kernel`，Error 218）。
- 推断：默认 `+initmem`/`+initreg` 的 spec 字符未覆盖到 prefetch FIFO 这类结构
  （Questa 的 `+initmem[=<spec>]` 用 spec 字符控制初始化范围）。要全覆盖需逐一精调
  spec，每次迭代都要重新 elaborate + run（很慢），ROI 低。
- 现状处理：把该选项改为**默认关闭的可选开关** `INIT_OPTS ?=`（保留能力、避免默认
  崩溃），并把结论写入 `sim/Makefile` 注释。需要时可
  `make elaborate INIT_OPTS="+initreg+0 +initmem+0"`。

**后续可选方向**（均不属于加速器范围，且不保证成功）：

1. 在 `vlog`（compile 阶段）也加初始化选项（二进制 help 提示「applied to vlog/vopt」）。
2. 精调 `+initmem=<spec>+0` 的 spec 字符以覆盖 prefetch FIFO 数组。
3. 由 SoC owner 在 RTL/复位序列层面消除取指路径 X（治本，需改 SoC 基础设施）。

**结论**：QuestaSim 全 SoC 流程已全部打通（许可证、脚本、Makefile、完成握手、断言
控制），唯一阻塞是 SoC/厂商 IP 的 4-state 取指 X，**不在加速器验证范围内**。
**功能签核以 Verilator 为准**；QuestaSim 用于编译 / elaboration / 集成覆盖。
...
| 文件 | 变更 |
|------|------|
| `scripts/c_code/gen_accel_data.py` | 新增：随机 A/B + 黄金参考生成器 |
| `Didactic-SoC/sw/accel/accel_gemm_data.h` | 新增（生成）：测试向量与黄金矩阵 |
| `Didactic-SoC/sw/accel/accel.c` | 改：随机数据 + 黄金比对；`main` 改为 `return` |
| `Didactic-SoC/sim/Makefile` | 改：`RUN_CMD`、`COMMON_CELLS_ASSERTS_OFF`、可选 `INIT_OPTS` |
...
- **加速器在 SoC 级的功能正确性已经签核**：Verilator 全 SoC 下内核经真实 OBI/APB
  ...
  逐字节一致的固件佐证。
- **QuestaSim 全 SoC 流程已完全可用**（编译/elaboration/许可证/加载/握手/断言控制
  均已解决），仅余 SoC/厂商 IP 的 **4-state 取指 X 传播**这一基础设施性阻塞。
- **建议**：以 Verilator 作为功能签核手段；QuestaSim 用于编译与集成覆盖。若需
  QuestaSim 也跑到 PASS，应由 SoC owner 从 RTL/复位或 `+initmem` spec 精调层面消除
  取指 X（§5.1 的后续方向）。
