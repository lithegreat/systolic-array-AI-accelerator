...
| TD-7 | RTL / control_unit | Found by formal verification (SymbiYosys k-induction): if `CTRL.softrst` is set on the same cycle a compute transition would otherwise fire, `cstate_q` is correctly forced back to `IDLE`, but `reg_status[STATUS_BUSY_BIT]` is still assigned from the pre-override `cstate_d`, so `STATUS.busy` reads `1` for one cycle while the FSM is already `IDLE`. Not yet fixed in RTL; the formal property is deliberately qualified to exclude this cycle rather than hide it. | A driver polling `STATUS.busy` right after issuing a soft-reset can observe a stale "busy" for one cycle. Low severity (self-corrects next cycle) but a real functional bug. | [control_unit_formal.md §4](../verification/control_unit_formal.md#4-a-real-bug-found-this-way) |
